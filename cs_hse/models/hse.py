...
class GteFlha(models.Model):
    ...
    hazard_ids = fields.One2many("cs.flha.hazard", "flha_id", copy=True)
    signoff_ids = fields.One2many("cs.flha.signoff", "flha_id")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                ...
                vals["name"] = self._cs_next_number(project, "cs.flha", "FLHA")
...
class GteFlhaHazard(models.Model):
    _name = "cs.flha.hazard"
    _description = "FLHA Hazard Line"

    flha_id = fields.Many2one("cs.flha", required=True, ondelete="cascade")
...
class GteToolboxTalk(models.Model):
    ...
    notes = fields.Html()
    attendee_ids = fields.One2many("cs.toolbox.attendee", "talk_id")
    state = fields.Selection([
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.toolbox.talk", "TBT")
        return super().create(vals_list)
...
class GteToolboxAttendee(models.Model):
    _name = "cs.toolbox.attendee"
    _description = "Toolbox Talk Attendee"

    talk_id = fields.Many2one("cs.toolbox.talk", required=True, ondelete="cascade")
    worker_name = fields.Char(required=True)
    signature = fields.Binary(string="Signature")

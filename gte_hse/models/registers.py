    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                ...
                vals["name"] = self._gte_next_number(project, "gte.incident", "INC")
...
class GteEquipmentInspection(models.Model):
    _name = "gte.equipment.inspection"
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._gte_next_number(
                    project, "gte.equipment.inspection", "EQI")
        return super().create(vals_list)
...
class GteWorkPermit(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._gte_next_number(project, "gte.work.permit", "WP")
        return super().create(vals_list)
...
class GteRisk(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._gte_next_number(project, "gte.risk", "RSK")
        return super().create(vals_list)

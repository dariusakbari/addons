...
class GteDailyLog(models.Model):
    ...
    labour_ids = fields.One2many("gte.daily.log.labour", "log_id")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                ...
                vals["name"] = self._gte_next_number(project, "gte.daily.log", "DSL")
...
class GteDailyLogLabour(models.Model):
    _name = "gte.daily.log.labour"
    _description = "Daily Log Labour Line"

    log_id = fields.Many2one("gte.daily.log", required=True, ondelete="cascade")

...
class GtePunchItem(models.Model):
    ...
    photo_before_ids = fields.Many2many(
        "ir.attachment", "gte_punch_before_rel", string="Before Photos")
    photo_after_ids = fields.Many2many(
        "ir.attachment", "gte_punch_after_rel", string="After Photos")
    ...
    inspection_id = fields.Many2one("gte.inspection", string="From Inspection")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                ...
                vals["name"] = self._gte_next_number(project, "gte.punch.item", "PL")
...
class GteMeeting(models.Model):
    ...
    action_ids = fields.One2many("gte.meeting.action", "meeting_id")
    state = fields.Selection([
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._gte_next_number(project, "gte.meeting", "MIN")
        return super().create(vals_list)
...
class GteMeetingAction(models.Model):
    _name = "gte.meeting.action"
    _description = "Meeting Action Item"
    _order = "due_date"

    meeting_id = fields.Many2one("gte.meeting", required=True, ondelete="cascade")
    ...
    state = fields.Selection([("open", "Open"), ("done", "Done")],
                             default="open")
    related_rfi_id = fields.Many2one("gte.rfi", string="Related RFI")
    related_co_id = fields.Many2one("gte.change.order", string="Related Change")


class GteCloseoutItem(models.Model):
    _name = "gte.closeout.item"
...
class GteInspection(models.Model):
    _inherit = "gte.inspection"

    punch_item_ids = fields.One2many("gte.punch.item", "inspection_id",
                                     string="Deficiencies")
    ...
    def action_view_deficiencies(self):
        self.ensure_one()
        return {
            "type": "ir.actions.act_window", "res_model": "gte.punch.item",

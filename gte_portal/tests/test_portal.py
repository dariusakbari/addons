...
class TestGtePortal(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.shared = cls.env["gte.rfi"].create({
            "subject": "Shared RFI", "project_id": cls.project.id,
            "gte_portal_partner_ids": [(4, cls.consultant.id)]})
        cls.private = cls.env["gte.rfi"].create({
            "subject": "Private RFI", "project_id": cls.project.id})

    def test_portal_sees_only_shared(self):
        RfiPortal = self.env["gte.rfi"].with_user(self.portal_user)
    ...
    def test_response_logging(self):
        ...
        self.assertEqual(self.shared.gte_portal_responded_by_id, self.consultant)
        self.assertIn("A-3", self.shared.gte_portal_response)

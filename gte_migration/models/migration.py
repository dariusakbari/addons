...
class GteMigrationWizard(models.TransientModel):
    """Converts tagged project tasks into dedicated GTE control records.
    ...
    _name = "gte.migration.wizard"
    _description = "GTE Tagged-Task Migration"
    ...
    def _xmlid_exists(self, name):
        return bool(self.env["ir.model.data"].sudo().search_count(
            [("module", "=", "__gte_migration__"), ("name", "=", name)]))
    ...
    def action_run(self):
        ...
        for task in tag_tasks("RFI"):
            ...
            if not self.dry_run:
                ...
                rec = self.env["gte.rfi"].create(vals)
        ...
        for task in tag_tasks("Change Order"):
            ...
            if not self.dry_run:
                ...
                rec = self.env["gte.change.order"].create(vals)
        ...
        for task in tag_tasks("Submittal"):
            ...
            if not self.dry_run:
                ...
                if parsed.get("sb_spec"):
                    spec = self.env["gte.submittal.spec"].search(
                ...
                rec = self.env["gte.submittal"].create(vals)
        ...
        if "x_submittal_spec" in self.env:
            for old in self.env["x_submittal_spec"].sudo().search([]):
                ...
                if not self.dry_run:
                    ...
                    rec = self.env["gte.submittal.spec"].create({
                        "number": str(number),
        ...
        self.result = "\n".join(lines)
        _logger.info("GTE migration: %s", self.result)
        return {

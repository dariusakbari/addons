    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                ...
                vals["name"] = self._cs_next_number(project, "cs.ncr", "NCR")
...
class GteInspection(models.Model):
    ...
    line_ids = fields.One2many("cs.inspection.line", "inspection_id", copy=True)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.inspection", "GI")
        return super().create(vals_list)
...
class GteInspectionLine(models.Model):
    _name = "cs.inspection.line"
    _description = "Inspection Checklist Item"

    inspection_id = fields.Many2one("cs.inspection", required=True,
                                    ondelete="cascade")
...
class GteAttendance(models.Model):
    ...
    line_ids = fields.One2many("cs.site.attendance.line", "sheet_id")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.site.attendance", "ATT")
        return super().create(vals_list)
...
class GteAttendanceLine(models.Model):
    _name = "cs.site.attendance.line"
    _description = "Attendance Line"

    sheet_id = fields.Many2one("cs.site.attendance", required=True,
                               ondelete="cascade")
...
class GteVisitor(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.visitor.log", "VIS")
        return super().create(vals_list)
...
class GteTransmittal(models.Model):
    ...
    line_ids = fields.One2many("cs.transmittal.line", "transmittal_id", copy=True)
    attachment_ids = fields.Many2many("ir.attachment", string="Files")
    distribution_ids = fields.Many2many("res.partner",
                                        "cs_transmittal_dist_rel", string="CC")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.transmittal", "TR")
        return super().create(vals_list)
...
class GteTransmittalLine(models.Model):
    _name = "cs.transmittal.line"
    _description = "Transmittal Item"

    transmittal_id = fields.Many2one("cs.transmittal", required=True,
                                     ondelete="cascade")
...
class GteShopDrawing(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.shop.drawing", "SD")
        return super().create(vals_list)
...
class GteGatePass(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                project = self.env["project.project"].browse(vals["project_id"])
                vals["name"] = self._cs_next_number(project, "cs.gate.pass", "GP")
        return super().create(vals_list)

# -*- coding: utf-8 -*-
import json
import logging
from datetime import datetime, date

import requests

from odoo import api, fields, models, _
from odoo.exceptions import ValidationError, UserError, RedirectWarning

_logger = logging.getLogger(__name__)


class ProductCategory(models.Model):
    _inherit = 'product.category'

    qbo_product_category_id = fields.Char(
        "QBO Category Id", copy=False, help="QuickBooks database recordset id")

    @api.model
    def get_category_ref(self, qbo_categ_id):
        try:
            company = self.env['res.users'].search(
                [('id', '=', self._uid)]).company_ids
            if not company:
                raise ValidationError(f"Company not found for the current user while processing QBO Category ID: {qbo_categ_id}")
            
            categ = self.search(
                [('qbo_product_category_id', '=', qbo_categ_id)], limit=1)
            # If account is not created in odoo then import from QBO and create.
            if not categ:
                try:
                    url_str = company.get_import_query_url()
                    url = url_str.get('url') + '/item/' + qbo_categ_id + \
                        '?minorversion=' + url_str.get('minorversion')
                    data = requests.request('GET', url, headers=url_str.get('headers'))
                except Exception as e:
                    raise ValidationError(f"Failed to fetch data from QBO API for QBO Category ID: {qbo_categ_id}. Error: {str(e)}")
                if data.status_code == 200:
                    try:
                        categ = self.create_product_category(data)
                    except Exception as e:
                        raise ValidationError(f"Failed to create product category for QBO Category ID: {qbo_categ_id}. Error: {str(e)}")
                else:
                    raise ValidationError(f"QBO API request failed with status code {data.status_code} for QBO Category ID: {qbo_categ_id}")

            if categ:
                return categ.id
            else:
                return False
        except ValidationError:
            raise  # re-raise to allow Odoo to handle it properly
        except Exception as e:
            raise ValidationError(f"Unexpected error occurred while processing QBO Category ID: {qbo_categ_id}. Error: {str(e)}")

    @api.model
    def create_product_category(self, data, parent=False):
        """Create product category object in odoo
        :param data: product category object response return by QBO
        :return product.category: product category object
        """
        try:
            company = self.env['res.users'].search(
                [('id', '=', self._uid)]).company_id
            _logger.info("COMPANY CATEGORY ------{} :".format(company))

            try:
                res = json.loads(str(data.text))
            except Exception as e:
                raise ValidationError("Failed to parse JSON from QBO response: %s" % str(e))

            categ_obj = False
            if 'QueryResponse' in res:
                categories = res.get('QueryResponse').get('Item', [])
            else:
                categories = [res.get('Item')] or []
            if len(categories) == 0:
                raise UserError("It seems that all of the Category are already imported.")
            max_result = res.get('QueryResponse').get('maxResults')

            for category in categories:
                qbo_id = category.get("Id", "Unknown")
                try:
                    if 'Type' in category and category.get('Type') == 'Category':
                        _logger.info("Type is category")
                    else:
                        _logger.info("Type is not category")
                        continue

                    if 'ParentRef' in category:
                        try:
                            categ_id = self.create_category_recursively(category)
                            categ_obj = self.browse(categ_id)
                        except Exception as e:
                            raise ValidationError("Failed to create parent category for QBO ID %s: %s" % (qbo_id, str(e)))
                    else:
                        # check if not category present then create otherwise use the
                        # same
                        vals = {
                            'name': category.get("Name", ''),
                            'qbo_product_category_id': category.get("Id"),
                        }
                        # If Income or expense account present then map them with odoo
                        # Income or expense account
                        if 'IncomeAccountRef' in category:
                            try:
                                if 'value' in category.get('IncomeAccountRef'):
                                    #                         account_id = self.get_income_account_ref(account_id)
                                    account_id = self.env['account.account'].get_account_ref(
                                        category.get('IncomeAccountRef').get('value'))
                                    vals.update(
                                        {'property_account_income_categ_id': account_id})
                            except Exception as e:
                                raise ValidationError("Failed to map Income Account for QBO ID %s: %s" % (qbo_id, str(e)))
                        if 'ExpenseAccountRef' in category:
                            #                     account_id = self.get_income_account_ref(category)
                            try:
                                if 'value' in category.get('IncomeAccountRef'):
                                    account_id = self.env['account.account'].get_account_ref(
                                        category.get('ExpenseAccountRef').get('value'))
                                    vals.update(
                                        {'property_account_expense_categ_id': account_id})
                            except Exception as e:
                                raise ValidationError("Failed to map Expense Account for QBO ID %s: %s" % (qbo_id, str(e)))
                            
                        categ_obj = self.search(
                            ['|', ('qbo_product_category_id', '=', category.get("Id")),
                            ('name', '=', category.get("Name", ''))], limit=1)

                        if not categ_obj:
                            try:
                                categ_obj = self.create(vals)
                                _logger.info(
                                    _("Product category created sucessfully! Category Id: %s" % (categ_obj.id)))
                                company = self.env.user.company_id
                                if company.import_pc_by_date:
                                    date_format = '%Y-%m-%d'
                                    try:
                                        if company.prodct_catgry_import_by == 'crt_dt':
                                            date_string = category.get('MetaData').get(
                                                'CreateTime')[:10]
                                        else:
                                            date_string = category.get('MetaData').get(
                                                'LastUpdatedTime')[:10]

                                        date_object = datetime.strptime(date_string,
                                                                        date_format).date()
                                        company.import_pc_date = date_object
                                    except Exception as e:
                                        raise ValidationError("Date parsing error for QBO ID %s: %s" % (qbo_id, str(e)))
                                self._cr.commit()
                            except Exception as e:
                                raise ValidationError("Failed to create product category for QBO ID %s: %s" % (qbo_id, str(e)))
                        else:
                            try:
                                categ_obj.write(vals)
                                _logger.info("Product category was written")
                                self._cr.commit()
                            except Exception as e:
                                raise ValidationError("Failed to update product category for QBO ID %s: %s" % (qbo_id, str(e)))
                            
                except ValidationError as ve:
                    _logger.error("ValidationError for QBO ID %s: %s", qbo_id, str(ve))
                    raise
                except Exception as e:
                    _logger.error("Unexpected error for QBO ID %s: %s", qbo_id, str(e))
                    raise ValidationError("Unexpected error occurred while processing QBO ID %s: %s" % (qbo_id, str(e)))
            return max_result
        except Exception as e:
            _logger.error("Error in create_product_category: %s", str(e))
            raise

    @api.model
    def create_category_recursively(self, category):
        try:
            if not 'ParentRef' in category:
                # Create parent category
                # check if not category present then create otherwise use the same
                vals = {
                    'name': category.get("Name", ''),
                    'qbo_product_category_id': category.get("Id"),
                }
                # If Income or expense account present then map them with odoo
                # Income or expense account
                try:
                    if 'IncomeAccountRef' in category:
                        account_id = self.env['account.account'].get_account_ref(
                            category.get('IncomeAccountRef').get('value'))
                        vals.update({'property_account_income_categ_id': account_id})
                    if 'ExpenseAccountRef' in category:
                        account_id = self.env['account.account'].get_account_ref(
                            category.get('ExpenseAccountRef').get('value'))
                        vals.update({'property_account_expense_categ_id': account_id})
                except Exception as e:
                    raise ValidationError(f"Failed to map income/expense accounts for QBO Category ID: {category.get('Id')}. Error: {str(e)}")

                try:
                    categ_obj = self.search(
                        ['|', ('qbo_product_category_id', '=', category.get("Id")),
                        ('name', '=', category.get("Name", ''))], limit=1)
                    if not categ_obj:
                        categ_obj = self.create(vals)
                    else:
                        categ_obj.write(vals)

                    self.env.cr.commit()
                    return categ_obj.id
                except Exception as e:
                    raise ValidationError(f"Error creating/updating category for QBO ID: {category.get('Id')}. Error: {str(e)}")
                
            else:
                try:
                    # read category object from QBO
                    company = self.env['res.users'].search(
                        [('id', '=', self._uid)]).company_id

                    url_str = company.get_import_query_url()
                    url = url_str.get(
                        'url') + '/item/%s' % category.get('ParentRef').get('value')
                    data = requests.request('GET', url, headers=url_str.get('headers'))
                    parent_category = json.loads(str(data.text))
                    self.env.cr.commit()
                except Exception as e:
                    raise ValidationError(f"Failed to fetch parent category from QBO for QBO Category ID: {category.get('Id')}. Error: {str(e)}")

                # If Income or expense account present then map them with odoo
                # Income or expense account
                income_account_id = False
                expense_account_id = False

                try:
                    if 'IncomeAccountRef' in category:
                        income_account_id = self.env['account.account'].get_account_ref(
                            category.get('IncomeAccountRef').get('value'))
                    #                 vals.update({'property_account_income_categ_id': account_id})
                    if 'ExpenseAccountRef' in category:
                        expense_account_id = self.env['account.account'].get_account_ref(
                            category.get('ExpenseAccountRef').get('value'))
                except Exception as e:
                    raise ValidationError(f"Failed to map income/expense accounts for QBO Category ID: {category.get('Id')}. Error: {str(e)}")
                
                #                 vals.update({'property_account_expense_categ_id': account_id})
                try:
                    categ_obj_new = self.search(
                        ['|', ('qbo_product_category_id', '=', category.get("Id")),
                         ('name', '=', category.get("Name", ''))], limit=1)
                    if categ_obj_new:
                        return categ_obj_new
                    return self.create({'name': category.get('Name'),
                                        'qbo_product_category_id': category.get("Id"),
                                        'property_account_income_categ_id': income_account_id,
                                        'property_account_expense_categ_id': expense_account_id,
                                        'parent_id': self.create_category_recursively(parent_category.get('Item'))}).id
                except Exception as e:
                    raise ValidationError(f"Error creating child category for QBO ID: {category.get('Id')}. Error: {str(e)}")
        except Exception as e:
            raise ValidationError(f"Unhandled error for QBO Category ID: {category.get('Id')}. Error: {str(e)}")


class Product(models.Model):
    _inherit = "product.template"

    # related to display product product information if is_product_variant
    qbo_product_id = fields.Char(
        'QBO Product Id', related='product_variant_ids.qbo_product_id', help="")

    x_is_exported = fields.Boolean('is_exported', default=False)


    # company name will be set while creating new product
    @api.model_create_multi
    def create(self, vals):
        for val in vals:
            if not val.get('company_id'):
                val['company_id'] = self.env.user.company_id.id
        res = super(Product, self).create(vals)
        return res


    def get_asset_account_ref(self):
        company = self.env['res.users'].search(
            [('id', '=', self._uid)]).company_id
        access_token = None
        realmId = None

        if company.access_token:
            access_token = company.access_token
        if company.realm_id:
            realmId = company.realm_id
        if access_token:
            headers = {}
            headers['Authorization'] = 'Bearer ' + str(access_token)
            headers['Content-Type'] = 'application/json'
            headers['accept'] = 'application/json'
            result = requests.request('GET',
                                      company.url + str(
                                          realmId) + "/query?query=select name,acctnum from account where Name like 'Inventory Asset'",
                                      headers=headers)
            if result.status_code == 200:
                resp = json.loads(result.text)
                if resp.get('QueryResponse').get('Account')[0]:
                    data_dict = {
                        'name': resp.get('QueryResponse').get('Account')[0].get('Name'),
                        'value': resp.get('QueryResponse').get('Account')[0].get('Id')
                    }
                    return data_dict
                else:
                    return False
            else:
                return False

    @api.model
    def get_qbo_product_ref(self, product):
        from_button = self._context.get('from_button', False)
        if product.qbo_product_id:
            return product.qbo_product_id
        else:
            #             product_tmpl = self.search([('id','=',product.product_tmpl_id.id)])
            product.export_product_to_qbo()
            if product.qbo_product_id:
                return product.qbo_product_id
            else:
                if from_button:
                    self.env['qbo.logger'].sudo().create({
                        'odoo_name': f'{self.name}',
                        'odoo_object': 'product.template',
                        'message': "Product not exported to QBO.",
                        'activity': 'Export from Odoo',
                        'created_date': fields.Datetime.now(),
                    })
                else:
                    raise ValidationError(_("Product not exported to QBO."))

    def getSyncToken(self, item_id):
        company = self.env['res.users'].search(
            [('id', '=', self._uid)]).company_id
        access_token = None
        realmId = None
        # Get SyncToken and of Id
        sql_query = "select Id,SyncToken from item Where Id = '{}'".format(
            str(item_id))

        if company.access_token:
            access_token = company.access_token
        if company.realm_id:
            realmId = company.realm_id

        if access_token:
            headers = {}
            headers['Authorization'] = 'Bearer ' + str(access_token)
            headers['Content-Type'] = 'application/json'
            headers['accept'] = 'application/json'

        result = requests.request(
            'GET', company.url + str(realmId) + "/query?query=" + sql_query, headers=headers)
        if result.status_code == 200:
            parsed_result = result.json()
            if parsed_result.get('QueryResponse') and parsed_result.get('QueryResponse').get('Item'):
                ''' GET SYNC TOKEN'''
                syncToken = parsed_result.get('QueryResponse').get(
                    'Item')[0].get('SyncToken')
                return syncToken
            else:
                return False
        else:
            return False

    def export_product_to_qbo(self):
        from_button = self._context.get('from_button', False)
        access_token = None
        realmId = None
        for product_id in self:
            try:
                ''' Check If expense and income account is set or not '''

                company = self.env['res.users'].search(
                    [('id', '=', self._uid)]).company_id


                if not product_id.qbo_product_id:
                    if not product_id.property_account_income_id:
                        if not product_id.categ_id.property_account_income_categ_id:
                            if from_button:
                                self.env['qbo.logger'].sudo().create({
                                    'odoo_name': f'{product_id.name}',
                                    'odoo_object': 'product.product',
                                    'message': 'Please Set Income Account for Product or Product Category: {}'.format(product_id.name),
                                    'activity': 'Exporting Product from Odoo',
                                    'created_date': fields.Datetime.now(),
                                })
                            else:
                                raise ValidationError(
                                    'Please Set Income Account for Product or Product Category: {}'.format(product_id.name))
                    if not product_id.property_account_expense_id:
                        if not product_id.categ_id.property_account_expense_categ_id:
                            if from_button:
                                self.env['qbo.logger'].sudo().create({
                                    'odoo_name': f'{product_id.name}',
                                    'odoo_object': 'product.product',
                                    'message': 'Please Set Expense Account for Product or Category: {}'.format(product_id.name),
                                    'activity': 'Exporting Product from Odoo',
                                    'created_date': fields.Datetime.now(),
                                })
                            else:
                                raise ValidationError(
                                    'Please Set Expense Account for Product or Category: {}'.format(product_id.name))

                if product_id.property_account_income_id and not product_id.property_account_income_id.qbo_id:
                    #     export account to quickbooks
                    product_id.property_account_income_id.export_single_account()

                if product_id.property_account_expense_id and not product_id.property_account_expense_id.qbo_id:
                    #     export account to quickbooks
                    product_id.property_account_expense_id.export_single_account()
                # account from product category .......................
                if not product_id.property_account_income_id:
                    if product_id.categ_id.property_account_income_categ_id and not product_id.categ_id.property_account_income_categ_id.qbo_id:
                        product_id.categ_id.property_account_income_categ_id.export_single_account()
                if not product_id.property_account_expense_id:
                    if product_id.categ_id.property_account_expense_categ_id and not product_id.categ_id.property_account_expense_categ_id.qbo_id:
                        product_id.categ_id.property_account_expense_categ_id.export_single_account()

                vals = {
                    "Name": product_id.name,


                    "UnitPrice": product_id.list_price,

                    "InvStartDate": str(date.today())
                }


                if product_id.property_account_expense_id and product_id.property_account_expense_id.qbo_id:
                    vals.update({"ExpenseAccountRef": {
                        "value": product_id.property_account_expense_id.qbo_id
                    }, })
                if product_id.property_account_income_id and product_id.property_account_income_id.qbo_id:
                    vals.update({"IncomeAccountRef": {
                        "value": product_id.property_account_income_id.qbo_id

                    }, })

                if not product_id.property_account_expense_id:
                    if product_id.categ_id.property_account_expense_categ_id and product_id.categ_id.property_account_expense_categ_id.qbo_id:
                        vals.update({"ExpenseAccountRef": {
                            "value": product_id.categ_id.property_account_expense_categ_id.qbo_id
                        }, })

                if not product_id.property_account_income_id:
                    if product_id.categ_id.property_account_income_categ_id and product_id.categ_id.property_account_income_categ_id.qbo_id:
                        vals.update({"IncomeAccountRef": {
                            "value": product_id.categ_id.property_account_income_categ_id.qbo_id
                        }, })

                if product_id.standard_price:
                    vals.update({'PurchaseCost': product_id.standard_price})

                if product_id.description_sale:
                    vals.update({
                        'Description': product_id.description_sale
                    })

                if product_id.default_code:
                    vals.update({'Sku': product_id.default_code})

                if product_id.description_purchase:
                    vals.update({'PurchaseDesc': product_id.description_purchase})

                if product_id.type == "consu":
                    vals.update({
                        'Type': 'NonInventory'
                    })

                if product_id.type == "service":
                    vals.update({
                        'Type': 'Service'
                    })

                if product_id.type == "consu" and product_id.is_storable:    # changed product to consu for storabale product (Inventory)
                    if company.non_tracked_item:
                        vals.update({
                            'Type': 'NonInventory'
                        })
                    else:
                        # Get quickbooks id of inventory asset COA from odoo
                        inv_asset = self.env['account.account'].search(
                            [('name', 'like', 'Inventory Asset')], limit=1)
                        vals.update({
                            "QtyOnHand": product_id.qty_available,
                            'Type': 'Inventory',
                            'TrackQtyOnHand': True
                        })
                        if inv_asset and inv_asset.qbo_id:
                            vals.update({
                                'AssetAccountRef': {
                                    'value': inv_asset.qbo_id
                                }
                            })

                if product_id.categ_id.qbo_product_category_id:
                    vals.update({
                        'SubItem': True,
                        'ParentRef': {
                            'value': product_id.categ_id.qbo_product_category_id
                        }
                    })

                if company.access_token:
                    access_token = company.access_token
                if company.realm_id:
                    realmId = company.realm_id

                result = False
                if access_token:
                    headers = {}
                    headers['Authorization'] = 'Bearer ' + str(access_token)
                    headers['Content-Type'] = 'application/json'
                    headers['accept'] = 'application/json'

                    # if product_id.qbo_product_id and product_id.x_is_exported:
                    if product_id.qbo_product_id:
                        # update_product = self.env['ir.config_parameter'].sudo().get_param(
                        #     'pragmatic_quickbooks_connector_canada.update_product_export')
                        company = self.env['res.users'].search(
                            [('id', '=', self._uid)]).company_id
                        update_product = company.update_product_export
                        if update_product:
                            vals.update({'sparse': True})
                            synctoken = self.getSyncToken(
                                product_id.qbo_product_id)

                            if synctoken:
                                vals.update({'Id': product_id.qbo_product_id})
                                vals.update({'SyncToken': synctoken})
                                # to update qty on hand
                                parsed_dict = json.dumps(vals)
                                _logger.info(
                                    'Parsed Dict for update: {}'.format(parsed_dict))
                                result = requests.request('POST', company.url + str(
                                    realmId) + "/item?operation=update&minorversion=75", headers=headers,
                                                        data=parsed_dict)

                                if result.status_code == 400:
                                    response = json.loads(result.text)
                                    if response.get('Fault'):
                                        if response.get('Fault').get('Error'):
                                            for message in response.get('Fault').get('Error'):
                                                if message.get('Detail'):
                                                    if from_button:
                                                        self.env['qbo.logger'].sudo().create({
                                                            'odoo_name': 'Quickbooks Online Exception',
                                                            'odoo_object': 'product.product',
                                                            'message': 'Quickbooks Online Exception \n\n' + message.get('Detail'),
                                                            'activity': 'Exporting Product from Odoo',
                                                            'created_date': fields.Datetime.now(),
                                                        })
                                                    else:
                                                        raise UserError(
                                                            'Quickbooks Online Exception \n\n' + message.get('Detail'))
                    else:
                        parsed_dict = json.dumps(vals)
                        _logger.info(
                            'Parsed Dict for create: {}'.format(parsed_dict))
                        result = requests.request('POST', company.url + str(realmId) + "/item?minorversion=75",
                                                headers=headers, data=parsed_dict)
                        if result.status_code == 400:
                            response = json.loads(result.text)
                            if response.get('Fault'):
                                if response.get('Fault').get('Error'):
                                    for message in response.get('Fault').get('Error'):
                                        if message.get('Detail'):
                                            if from_button:
                                                self.env['qbo.logger'].sudo().create({
                                                    'odoo_name': 'Quickbooks Online Exception',
                                                    'odoo_object': 'product.product',
                                                    'message': 'Quickbooks Online Exception \n\n' + message.get('Detail'),
                                                    'activity': 'Exporting Product from Odoo',
                                                    'created_date': fields.Datetime.now(),
                                                })
                                            else:
                                                raise UserError(
                                                    'Quickbooks Online Exception  \n\n' + message.get('Detail'))
                    if result:
                        if result.status_code == 200:
                            resp_parsed = json.loads(result.text)
                            if resp_parsed.get('Item').get('Id'):
                                ''' Set is_exported to true and add reference of newely created procut in quickbooks'''
                                product_id.x_is_exported = True
                                product_id.qbo_product_id = resp_parsed.get(
                                    'Item').get('Id')
                                _logger.info(_("Product exported sucessfully! product template Id: %s" % (
                                    product_id.qbo_product_id)))
                                self._cr.commit()
            except Exception as e:
                _logger.exception("Unexpected error during QBO export")
                if from_button:
                    self.env['qbo.logger'].sudo().create({
                        'odoo_name': product_id.name,
                        'odoo_object': 'product.product',
                        'message': 'Unexpected Error: ' + str(e),
                        'activity': 'Exporting Product from Odoo',
                        'created_date': fields.Datetime.now(),
                    })
                else:
                    raise UserError("Unexpected Error: " + str(e))  

    @api.model
    def create_product(self, data, parent=False, call_from=None, company=None):
        """Create product object in odoo
        :param data: product object response return by QBO
        :return product.template: product template object
        """
        account = self.env['account.account']
        tax = self.env['account.tax']
        category = self.env['product.category']
        prod_obj = False
        try:
            res = json.loads(str(data.text))
        except Exception as e:
            raise ValidationError(_("Failed to parse product data JSON. Error: %s" % str(e)))

        if 'QueryResponse' in res:
            products = res.get('QueryResponse').get('Item', [])
        else:
            products = [res.get('Item')] or []

        if len(products) == 0 and not call_from:
            raise UserError(
                "It seems that all of the Products are already imported.")
        max_result = res.get('QueryResponse').get('maxResults')

        for product in products:
            try:
                if product.get('Type') == 'Service' or product.get('Type') == 'Inventory' or product.get(
                        'Type') == 'NonInventory':
                    product_type = 'consu'
                    is_storable = False
                    if product.get('Type') == 'NonInventory':
                        product_type = 'consu'
                    elif product.get('Type') == 'Inventory':
                        product_type = 'consu'  # Changed from 'product' to 'product' to represent goods
                        is_storable = True  # Set is_storable to True for Inventory items
                    elif product.get('Type') == 'Service':
                        product_type = 'service'


                    vals = {
                        'name': product.get('Name', ''),
                        'description_sale': product.get('Description', ''),
                        'description_purchase': product.get('PurchaseDesc', ''),
                        'list_price': product.get('UnitPrice', ''),
                        'standard_price': product.get('PurchaseCost', ''),
                        'default_code': product.get('Sku', ''),
                        'type': product_type,
                        # 'active': product.get('Active'),
                        'qbo_product_id': product.get('Id'),
                        'company_id': company.id,
                        'is_storable': is_storable,
                    }


                    categ_id = category.get_category_ref(
                        product.get('ParentRef').get('value')) if 'ParentRef' in product else False,


                    if categ_id and not False in categ_id:
                        vals.update({
                            'categ_id': int(categ_id[0])
                        })
                    else:
                        _logger.info(_("Categ ID was not found"))

                    # Attaching Income Account for a product
                    if not company:
                        company = self.env['res.users'].search(
                            [('id', '=', self._uid)]).company_id
                    if 'IncomeAccountRef' in product:
                        vals.update({'property_account_income_id': account.get_account_ref(
                            product.get('IncomeAccountRef').get('value'), company=company)})
                    else:
                        _logger.info(
                            "Income Account Reference not found,attaching from Default QBO Configuration")

                        if company.qb_income_account:
                            vals.update(
                                {'property_account_income_id': company.qb_income_account.id})
                        else:
                            raise ValidationError(_(
                                """Please set QBO Income Account!
                                Product QBO ID: %s
                                Navigate to Settings ---> Companies ---> Quickbooks Tab.
                                Under QB Account Configuration, please set Product Income Account."""
                            ) % product.get('Id'))

                    # Attaching Expense Account for a product
                    if 'ExpenseAccountRef' in product:
                        vals.update({'property_account_expense_id': account.get_account_ref(
                            product.get('ExpenseAccountRef').get('value'), company=company)})
                    else:
                        _logger.info(
                            "Expense Account Reference not found,attaching from Default QBO Configuration")

                        if not company:
                            company = self.env.company
                        if company.qb_expense_account:
                            vals.update(
                                {'property_account_expense_id': company.qb_expense_account.id})
                        else:
                            raise ValidationError(_(
                                """Please set QBO Expense Account!
                                Product QBO ID: %s
                                Navigate to Settings ---> Companies ---> Quickbooks Tab.
                                Under QB Account Configuration, please set Product Expense Account."""
                            ) % product.get('Id'))

                    if 'SalesTaxCodeRef' in product:
                        tax_id = tax.get_account_tax_ref(product.get('SalesTaxCodeRef').get('value'),
                                                        product.get(
                                                            'SalesTaxCodeRef').get('name'),
                                                        type_tax_use="sale", company=company)
                        if tax_id:
                            vals.update({'taxes_id': [(6, 0, [tax_id])]})
                    if 'PurchaseTaxCodeRef' in product:
                        tax_id = tax.get_account_tax_ref(product.get('PurchaseTaxCodeRef').get('value'),
                                                        product.get(
                                                            'PurchaseTaxCodeRef').get('name'),
                                                        type_tax_use="purchase", company=company)
                        if tax_id:
                            vals.update({'supplier_taxes_id': [(6, 0, [tax_id])]})

                    product_product = self.env['product.product']
                    if 'Sku' in product and product.get('Sku'):
                        prod_obj = product_product.search(
                            ['|', ('default_code', '=', product.get('Sku')),
                            ('qbo_product_id', '=', product.get("Id")),
                            ('active', 'in', [True, False]),
                            ('company_id', '=', company.id)], limit=1)

                            # ['|', ('default_code', '=', product.get('Sku')), ('qbo_product_id', '=', product.get("Id")),
                            #  ('company_id', '=', company.id)])
                    else:
                        prod_obj = product_product.search(
                            [('qbo_product_id', '=', product.get("Id")), ('active', 'in', [True, False]),
                            ('company_id', '=', company.id)])


                    if len(prod_obj) > 1:
                        raise ValidationError(_("Found multiple products with internal reference. Product QBO ID: %s. Names: %s" % (
                        product.get("Id"), str([p.name for p in prod_obj]))))

                    if not prod_obj:
                        created = prod_obj = prod_obj.create(vals)
                        if created:
                            _logger.info(
                                _("Product created sucessfully! product template Id: %s" % (prod_obj.id)))
                            if not company:
                                company = self.env.user.company_id
                            if company.import_product_by_date:
                                date_format = '%Y-%m-%d'
                                if company.prodct_import_by == 'crt_dt':
                                    date_string = product.get('MetaData').get(
                                        'CreateTime')[:10]
                                else:
                                    date_string = product.get('MetaData').get(
                                        'LastUpdatedTime')[:10]

                                date_object = datetime.strptime(date_string,
                                                                date_format).date()
                                company.import_product_date = date_object
                    else:
                        if not company:
                            company = self.env['res.users'].search(
                                [('id', '=', self._uid)]).company_id
                        update_product = company.update_product_import
                        if update_product:
                            if 'type' in vals:
                                del vals['type']
                            if 'name' in vals:
                                del vals['name']
                            updated = prod_obj.write(vals)
                            if updated:
                                _logger.info(
                                    _("Product updated sucessfully! product template Id: %s" % (prod_obj.id)))
                                if not company:
                                    company = self.env.user.company_id
                                if company.import_product_by_date:
                                    date_format = '%Y-%m-%d'
                                    if company.prodct_import_by == 'crt_dt':
                                        date_string = product.get('MetaData').get(
                                            'CreateTime')[:10]
                                    else:
                                        date_string = product.get('MetaData').get(
                                            'LastUpdatedTime')[:10]

                                    date_object = datetime.strptime(date_string,
                                                                    date_format).date()
                                    company.import_product_date = date_object
                    self.env.cr.commit()
            except ValidationError as ve:
                raise ve  # re-raise specific errors for context
            except Exception as e:
                raise ValidationError(_("An error occurred while processing product QBO ID: %s. Error: %s" % (
                    product.get('Id'), str(e))))
        return max_result


class ProductProduct(models.Model):
    _inherit = "product.product"

    qbo_product_id = fields.Char('QBO Product ID', help="Refer to QBO Item Id")
    is_discount_product = fields.Boolean(string="Is Discount Product", related='product_tmpl_id.is_discount_product',
                                         copy=False)

    def write(self, vals):
        if 'company_id' in vals and vals['company_id']:
            products_changing_company = self.filtered(lambda product: product.company_id.id != vals['company_id'])
            if products_changing_company:
                move = self.env['stock.move'].sudo().search([
                    ('product_id', 'in', products_changing_company.product_variant_ids.ids),
                    ('company_id', 'not in', [vals['company_id'], False]),
                ], order=None, limit=1)
                if move:
                    raise UserError(_("This product's company cannot be changed as long as there are stock moves of it belonging to another company."))

                # Forbid changing a product's company when quant(s) exist in another company.
                quant = self.env['stock.quant'].sudo().search([
                    ('product_id', 'in', products_changing_company.product_variant_ids.ids),
                    ('company_id', 'not in', [vals['company_id'], False]),
                    ('quantity', '!=', 0),
                ], order=None, limit=1)
                if quant:
                    raise UserError(_("This product's company cannot be changed as long as there are quantities of it belonging to another company."))

        if 'uom_id' in vals:
            new_uom = self.env['uom.uom'].browse(vals['uom_id'])
            updated = self.filtered(lambda template: template.uom_id != new_uom)
            done_moves = self.env['stock.move'].sudo().search([('product_id', 'in', updated.with_context(active_test=False).mapped('product_variant_ids').ids)], limit=1)
            if done_moves:
                raise UserError(_("You cannot change the unit of measure as there are already stock moves for this product. If you want to change the unit of measure, you should rather archive this product and create a new one."))

        if 'is_storable' in vals and not vals['is_storable'] and sum(self.mapped('nbr_reordering_rules')) != 0:
            raise UserError(_('You still have some active reordering rules on this product. Please archive or delete them first.'))

        if any('is_storable' in vals and vals['is_storable'] != prod_tmpl.is_storable for prod_tmpl in self):
            existing_done_move_lines = self.env['stock.move.line'].sudo().search([
                ('product_id', 'in', self.with_context(active_test=False).mapped('product_variant_ids').ids),
                ('state', '=', 'done'),
            ], limit=1)
            if existing_done_move_lines:
                product_name = existing_done_move_lines.product_id.name
                product_reference = existing_done_move_lines.product_id.default_code or "No Reference"
                raise UserError(_("You can not change the inventory tracking of a product that was already used.  Product: {} (Reference: {})".format(product_name, product_reference)))
            existing_reserved_move_lines = self.env['stock.move.line'].sudo().search([
                ('product_id', 'in', self.with_context(active_test=False).mapped('product_variant_ids').ids),
                ('state', 'in', ['partially_available', 'assigned']),
            ], limit=1)
            if existing_reserved_move_lines:
                product_name = existing_done_move_lines.product_id.name
                product_reference = existing_done_move_lines.product_id.default_code or "No Reference"
                raise UserError(_("You can not change the inventory tracking of a product that is currently reserved on a stock move. If you need to change the inventory tracking, you should first unreserve the stock move.  Product: {} (Reference: {})".format(product_name, product_reference)))

        if 'is_storable' in vals and not vals['is_storable'] and any(p.is_storable and not float_is_zero(p.qty_available, precision_rounding=p.uom_id.rounding) for p in self):
            raise UserError(_("Available quantity should be set to zero before changing inventory tracking"))

        return super(ProductProduct, self).write(vals)

    def check_product_by_sku_in_qbo(self, product_id):
        from_button = self._context.get('from_button', False)
        _logger.info(
            '\n\nInside To check SKU for product : {}'.format(product_id))
        company = self.company_id
        if not company:
            if from_button:
                self.env['qbo.logger'].sudo().create({
                    'odoo_name': f'{self.name}',
                    'odoo_object': 'Product',
                    'message': f'Set Company for Product: {self.name}',
                    'activity': 'Exporting Product from QBO',
                    'created_date': fields.Datetime.now(),
                })

                # Ensure the transaction is committed
                self.env.cr.commit()
            else:
                raise ValidationError(_(f"Set Company for Product {self.name}"))

        access_token = False
        realmId = False

        if company.access_token:
            access_token = company.access_token
        if company.realm_id:
            realmId = company.realm_id

        if access_token:
            headers = {}
            headers['Authorization'] = 'Bearer ' + str(access_token)
            headers['Content-Type'] = 'application/json'
            headers['accept'] = 'application/json'
            query = "select * from item where sku='" + \
                    str(product_id.default_code) + "'"
            url = company.url + \
                  str(realmId) + "/query?query=" + query + "&minorversion=75"
            _logger.info('\n\n\nQuery : {} \n\n URL : {}'.format(query, url))
            result = requests.request('GET', url, headers=headers)
            _logger.info(
                '\n\n\nResponse : {} -> {}'.format(result, result.text))

            if result:
                if result.status_code == 400:
                    response = json.loads(result.text)
                    if response.get('Fault'):
                        if response.get('Fault').get('Error'):
                            for message in response.get('Fault').get('Error'):
                                if message.get('Detail'):
                                    self.env['qbo.logger'].create({
                                        'odoo_name': product_id.name,
                                        'odoo_object': 'Product',
                                        'message': 'Quickbooks Online Exception \n\n' + message.get('Detail'),
                                        'created_date': datetime.now(),
                                    })
                                    self._cr.commit()
                                    raise UserError(
                                        'Quickbooks Online Exception \n\n' + message.get('Detail'))
                if result.status_code == 200:
                    resp_parsed = json.loads(result.text)
                    _logger.info('\n\n\nParsed Dict : {} '.format(resp_parsed))
                    resp_parsed = resp_parsed.get('QueryResponse')
                    if resp_parsed:
                        if resp_parsed.get('Item')[0].get('Id'):
                            ''' Set is_exported to true and add reference of newly created product in quickbooks'''
                            product_id.x_is_exported = True
                            product_id.qbo_product_id = resp_parsed.get(
                                'Item')[0].get('Id')
                            _logger.info(
                                _("Product Id Updated successfully! product template Id: %s" % (
                                    product_id.qbo_product_id)))
                            self._cr.commit()
                            return True

        return False

    # @api.multi
    def export_product_to_qbo(self, company=None, cron=None):
        from_button = self._context.get('from_button', False)
        if not company:
            company = self.env.company
        access_token = None
        realmId = None
        for product_id in self:
            ''' Check If expense and income account is set or not '''
            if not product_id.qbo_product_id and product_id.default_code:
                ret = self.check_product_by_sku_in_qbo(product_id)
                if ret:
                    _logger.info(
                        _("Product exported sucessfully! product template Id: %s" % (product_id.qbo_product_id)))
                    break
                    # return True
                else:
                    _logger.info(
                        'Product Not Found In qbo by sku id : {}'.format(ret))

            if not product_id.qbo_product_id:
                if not product_id.property_account_income_id:
                    if not product_id.categ_id.property_account_income_categ_id:
                        if from_button:
                            self.env['qbo.logger'].sudo().create({
                                'odoo_name': product_id.name,
                                'odoo_object': 'product.product',
                                'message': 'Please Set Income Account for Product or Product Category: {}'.format(product_id.name),
                                'activity': 'Exporting Product from Odoo',
                                'created_date': fields.Datetime.now(),
                            })
                        else:
                            raise ValidationError(
                                'Please Set Income Account for Product or Product Category: {}'.format(product_id.name))
                if not product_id.property_account_expense_id:
                    if not product_id.categ_id.property_account_expense_categ_id:
                        if from_button:
                            self.env['qbo.logger'].sudo().create({
                                'odoo_name': product_id.name,
                                'odoo_object': 'product.product',
                                'message': 'Please Set Expense Account for Product or Category: {}'.format(product_id.name),
                                'activity': 'Exporting Product from Odoo',
                                'created_date': fields.Datetime.now(),
                            })
                        else:
                            raise ValidationError(
                                'Please Set Expense Account for Product or Category: {}'.format(product_id.name))
            if company and not company.export_product_wo_sku and not product_id.default_code:
                if cron:
                    continue
                else:
                    # error_product_mca = self.env['qbo.logger'].sudo().create({
                    #     'odoo_name': f'{product_id.name}',
                    #     'odoo_object': 'product.product',
                    #     'message': f'Please Set Internal Reference for Product: {product_id.name}',
                    #     'activity': 'Exporting Product from Odoo',
                    #     'created_date': fields.Datetime.now(),
                    # })
                    # Ensure the transaction is committed
                    # self.env.cr.commit()
                    if from_button:
                        self.env['qbo.logger'].sudo().create({
                            'odoo_name': product_id.name,
                            'odoo_object': 'product.product',
                            'message': 'Please Set Internal Reference for Product: {}'.format(product_id.name),
                            'activity': 'Exporting Product from Odoo',
                            'created_date': fields.Datetime.now(),
                        })
                    else:
                        raise ValidationError(
                            'Please Set Internal Reference for Product: {}'.format(product_id.name))

            if product_id.property_account_income_id and not product_id.property_account_income_id.qbo_id:
                #     export account to quickbooks
                product_id.property_account_income_id.export_single_account()

            if product_id.property_account_expense_id and not product_id.property_account_expense_id.qbo_id:
                #     export account to quickbooks
                product_id.property_account_expense_id.export_single_account()
            # account from product category .......................
            if not product_id.property_account_income_id:
                if product_id.categ_id.property_account_income_categ_id and not product_id.categ_id.property_account_income_categ_id.qbo_id:
                    product_id.categ_id.property_account_income_categ_id.export_single_account()
            if not product_id.property_account_expense_id:
                if product_id.categ_id.property_account_expense_categ_id and not product_id.categ_id.property_account_expense_categ_id.qbo_id:
                    product_id.categ_id.property_account_expense_categ_id.export_single_account()


            if not company:
                company = self.company_id
            if not company:
                if from_button:
                    self.env['qbo.logger'].sudo().create({
                        'odoo_name': f'{self.name}',
                        'odoo_object': 'product.product',
                        'message': f"Set Company for Product {self.name}",
                        'activity': 'Exporting Product from Odoo',
                        'created_date': fields.Datetime.now(),
                    })
                else:
                    raise ValidationError(_(f"Set Company for Product {self.name}"))
            if not company:
                company = self.env.company
            d = datetime.strptime(str(date.today()), '%Y-%m-%d')
            as_of_today = date.strftime(d, "%d/%m/%y")
            #         Product details to be exported to Quickbooks

            product_name = product_id.name
            for prd in product_id.product_template_attribute_value_ids:
                product_name = product_name + ' ' + str(prd.name)

            vals = {
                "Name": product_name,

                "UnitPrice": product_id.list_price,
                "InvStartDate": str(date.today())
            }


            if product_id.property_account_expense_id and product_id.property_account_expense_id.qbo_id:
                vals.update({"ExpenseAccountRef": {
                    "value": product_id.property_account_expense_id.qbo_id
                }, })
            if product_id.property_account_income_id and product_id.property_account_income_id.qbo_id:
                vals.update({"IncomeAccountRef": {
                    "value": product_id.property_account_income_id.qbo_id

                }, })

            if not product_id.property_account_expense_id:
                if product_id.categ_id.property_account_expense_categ_id and product_id.categ_id.property_account_expense_categ_id.qbo_id:
                    vals.update({"ExpenseAccountRef": {
                        "value": product_id.categ_id.property_account_expense_categ_id.qbo_id
                    }, })

            if not product_id.property_account_income_id:
                if product_id.categ_id.property_account_income_categ_id and product_id.categ_id.property_account_income_categ_id.qbo_id:
                    vals.update({"IncomeAccountRef": {
                        "value": product_id.categ_id.property_account_income_categ_id.qbo_id
                    }, })

            if product_id.standard_price:
                vals.update({'PurchaseCost': product_id.standard_price})

            if product_id.description_sale:
                vals.update({
                    'Description': product_id.description_sale
                })

            if product_id.default_code:
                vals.update({'Sku': product_id.default_code})

            if product_id.description_purchase:
                vals.update({'PurchaseDesc': product_id.description_purchase})

            if product_id.type == "consu":
                vals.update({
                    'Type': 'NonInventory'
                })

            if product_id.type == "service":
                vals.update({
                    'Type': 'Service'
                })

            if product_id.type == "consu" and product_id.is_storable:
                if company.non_tracked_item:
                    vals.update({
                        'Type': 'NonInventory'
                    })
                else:
                    # Get quickbooks id of inventory asset COA from odoo
                    #                 inv_asset = self.env['account.account'].search([('name', 'like', 'Inventory Asset')], limit=1)
                    if not product_id.categ_id.property_stock_valuation_account_id:
                        if from_button:
                            self.env['qbo.logger'].sudo().create({
                                'odoo_name': product_id.name,
                                'odoo_object': 'product.product',
                                'message': f"Stock valuation account is missing for {product_id.default_code} {product_id.name}",
                                'activity': 'Exporting Product from Odoo',
                                'created_date': fields.Datetime.now(),
                            })
                        else:
                            raise UserError(_("Stock valuation account is missing for %s %s") % (product_id.default_code, product_id.name))
                    if not product_id.categ_id.property_stock_valuation_account_id.qbo_id:
                        product_id.categ_id.property_stock_valuation_account_id.export_single_account()

                    vals.update({
                        "QtyOnHand": product_id.qty_available,
                        'Type': 'Inventory',
                        'TrackQtyOnHand': True
                    })
                    if product_id.categ_id.property_stock_valuation_account_id.qbo_id:
                        vals.update({
                            'AssetAccountRef': {
                                'value': product_id.categ_id.property_stock_valuation_account_id.qbo_id
                            }
                        })

            if product_id.categ_id.qbo_product_category_id:
                vals.update({
                    'SubItem': True,
                    'ParentRef': {
                        'value': product_id.categ_id.qbo_product_category_id
                    }
                })

            if company.access_token:
                access_token = company.access_token
            if company.realm_id:
                realmId = company.realm_id
            result = False
            if access_token:
                headers = {}
                headers['Authorization'] = 'Bearer ' + str(access_token)
                headers['Content-Type'] = 'application/json'
                headers['accept'] = 'application/json'
                # if product_id.qbo_product_id and product_id.x_is_exported:
                if product_id.qbo_product_id:
                    company = self.env['res.users'].search(
                        [('id', '=', self._uid)]).company_id
                    update_product = company.update_product_export
                    if update_product:
                        vals.update({'sparse': True})
                        synctoken = self.product_tmpl_id.getSyncToken(
                            product_id.qbo_product_id)
                        if 'Name' in vals:
                            vals.pop('Name')
                        if synctoken:
                            vals.update({'Id': product_id.qbo_product_id})
                            vals.update({'SyncToken': synctoken})
                            # del vals['QtyOnHand'] Commented as API now allows
                            # to update qty on hand
                            parsed_dict = json.dumps(vals)
                            _logger.info(
                                'Parsed Dict for update product : {}'.format(parsed_dict))
                            result = requests.request('POST', company.url + str(
                                realmId) + "/item?operation=update&minorversion=75", headers=headers,
                                                      data=parsed_dict)
                            if result.status_code == 400:
                                response = json.loads(result.text)
                                if response.get('Fault'):
                                    if response.get('Fault').get('Error'):
                                        for message in response.get('Fault').get('Error'):
                                            if message.get('Detail'):
                                                if from_button:
                                                    self.env['qbo.logger'].create({
                                                        'odoo_name': product_id.name,
                                                        'odoo_object': 'Product',
                                                        'message': 'Quickbooks Online Exception \n\n' + message.get(
                                                            'Detail'),
                                                        'created_date': datetime.now(),
                                                    })
                                                else:
                                                    raise UserError(
                                                        'Quickbooks Online Exception \n\n' + message.get('Detail'))
                else:
                    parsed_dict = json.dumps(vals)
                    _logger.info(
                        'Parsed Dict for create product : {}'.format(parsed_dict))

                    result = requests.request('POST', company.url + str(realmId) + "/item?minorversion=75",
                                              headers=headers, data=parsed_dict)

                    if result.status_code == 400:
                        response = json.loads(result.text)
                        if response.get('Fault'):
                            if response.get('Fault').get('Error'):
                                for message in response.get('Fault').get('Error'):
                                    if message.get('Detail'):
                                        if from_button:
                                            self.env['qbo.logger'].create({
                                                'odoo_name': product_id.name,
                                                'odoo_object': 'Product',
                                                'message': 'Quickbooks Online Exception \n\n' + message.get('Detail'),
                                                'created_date': datetime.now(),
                                            })
                                        else:
                                            raise UserError(
                                                'Quickbooks Online Exception \n\n' + message.get('Detail'))
                if result:
                    if result.status_code == 200:
                        resp_parsed = json.loads(result.text)
                        if resp_parsed.get('Item').get('Id'):
                            ''' Set is_exported to true and add reference of newely created procut in quickbooks'''
                            product_id.x_is_exported = True
                            product_id.qbo_product_id = resp_parsed.get(
                                'Item').get('Id')
                            _logger.info(_("Product exported sucessfully! product template Id: %s" % (
                                product_id.qbo_product_id)))
                            self._cr.commit()



...
class GteReasonWizard(models.TransientModel):
    ...
    def action_apply(self):
        ...
        if self.action == "cancel":
            record.with_context(gte_reason=self.reason).action_cancel()
        elif self.action == "reopen":
            record.with_context(gte_reason=self.reason).action_reopen()
        elif self.action == "override":
            if not self.env.user.has_group("gte_core.group_gte_admin"):

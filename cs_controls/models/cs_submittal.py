...
class GteSubmittal(models.Model):
    ...
    spec_id = fields.Many2one("cs.submittal.spec", string="Standard Spec")
    ...
    revision_ids = fields.One2many("cs.submittal.revision", "submittal_id", copy=False)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("name", "New") == "New" and vals.get("project_id"):
                ...
                vals["name"] = self._cs_next_number(project, "cs.submittal", "SUB")
    ...
    def action_submit(self):
        ...
        for rec in self:
            rec.env["cs.submittal.revision"].create({
                "submittal_id": rec.id,
...
class GteSubmittalRevision(models.Model):
    _name = "cs.submittal.revision"
    _description = "Submittal Revision"
    _order = "submittal_id, revision"

    submittal_id = fields.Many2one("cs.submittal", required=True, ondelete="cascade")

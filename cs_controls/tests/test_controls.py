...
class TestGteControls(TransactionCase):
    ...
    def test_change_order_totals(self):
        ...
        self.env["cs.change.order.line"].create({
    ...
    def test_submittal_revisions_never_overwrite(self):
        ...
        self.env["ir.attachment"].create({
            "name": "spec.pdf", "res_model": "cs.submittal", "res_id": sub.id,
            "datas": "dGVzdA=="})
        sub.action_request()
        sub.action_receive()
        sub.action_review()
    ...
    def test_cross_project_uniqueness(self):
        ...
        r1 = self.env["cs.rfi"].create({"subject": "a", "project_id": self.project.id})
        r2 = self.env["cs.rfi"].create({"subject": "b", "project_id": other.id})

...
class TestGteMigration(TransactionCase):
    ...
    def _run(self, dry=False):
        wiz = self.env["cs.migration.wizard"].create({"dry_run": dry})
        wiz.action_run()
        return wiz
    ...
    def test_idempotent_and_non_destructive(self):
        ...
        self._run(dry=False)
        rfis = self.env["cs.rfi"].search([("origin_task_id", "=", self.task.id)])
    ...
    def test_dry_run_writes_nothing(self):
        self._run(dry=True)
        self.assertFalse(
            self.env["cs.rfi"].search([("origin_task_id", "=", self.task.id)]))

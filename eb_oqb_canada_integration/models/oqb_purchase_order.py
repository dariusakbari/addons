...
class PurchaseOrderInherit(models.Model):
    ...
    quickbook_purchase_order_name = fields.Char(string='Quickbook Purchase Order Name')

    def fetch_purchase_order_from_quickbook(self, current_instance, last_sync_date, operation_type):
        #     """
        #        Description:
        ...
        #        """
        return self.fetch_purchase_order_data_from_quickbook(current_instance, last_sync_date, None,
        'oqb.pco.lines','quickbook_purchase_order_dropdown_mapping',
        'PurchaseOrder','purchase order', operation_type)

        # --------------------------------- Sync Purchase Order Record Quickbook To Odoo ------------------------------- #

    def fetch_purchase_order_data_from_quickbook(self, current_instance, last_sync_date_field, is_company,
        ...
        self.env['oqb.dry.mixin'].fetch_data_from_quickbook(current_instance=current_instance,
            ...
            process_record_method='process_quickbook_purchase_order_record',
            sync_date_field='quickbook_purchase_order_last_sync_date',last_id_field='quickbook_purchase_order_last_id')

    # Sync Odoo to Quickbook Purchase Order Data

    def fetch_purchase_order_from_odoo(self, current_instance, last_sync_date, operation_type):
        return self.odoo_purchase_order_sync_to_quickbook(current_instance, last_sync_date, 'purchase.order', 'False',
                                                  'oqb.pco.lines', 'odoo_purchase_order_dropdown_mapping',
                                                  'PurchaseOrder', 'purchase order', operation_type)

    def odoo_purchase_order_sync_to_quickbook(self, current_instance, last_sync_date_field, odoo_module_name, is_company,
                                      field_model_name, dropdown_field_mapping_name, module_name, logger_name,
                                      operation_type):
        self.env['oqb.dry.mixin'].fetch_data_from_odoo(current_instance=current_instance,
            ...
            process_record_method='process_odoo_purchase_order_record',
            sync_date_field='odoo_purchase_order_last_sync_date',last_id_field='odoo_purchase_order_last_id')

    # -------------------- Sync Form and List View Odoo Purchase Orders Record to Quickbook ------------------- #

    def purchase_order_send_to_quickbook(self):
        # # Ensure active_ids are passed in correctly
        active_ids = self._context.get('active_ids', [])
        if not active_ids:
            active_ids = [self.id]
        success_count, active_ids, logger_name, operation_status = self.env[
            'oqb.dry.mixin'].odoo_record_send_to_quickbook('purchase order', active_ids, 'purchase.order',
                                                           'PurchaseOrder')
...
class PurchaseOrderLineInherit(models.Model):
    """
       Description:
           This class inherits the 'purchase.order.line' model and adds additional functionality for fetching
